...
int adventurerFunction(struct gameState *state, int handPos) {
...
int ghFunction(struct gameState *state, int handPos) {
  int currentPlayer = whoseTurn(state);
  int nextPlayer = currentPlayer + 1;
  if(nextPlayer > (state->numPlayers - 1)) {
    nextPlayer = 0;
...
int embargoFunction(struct gameState *state, int handPos, int choice1) {
  int currentPlayer = whoseTurn(state);
  int nextPlayer = currentPlayer + 1;
  if(nextPlayer > (state->numPlayers - 1)) {
    nextPlayer = 0;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    {
    case adventurer:
      adventurerFunction(state, handPos);
    ...
    case smithy:
      smithyFunction(state, handPos);

    case village:
      villageFunction(state, handPos);
    ...
    case embargo:
      embargoFunction(state, handPos, choice1);
